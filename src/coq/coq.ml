...
(** main procedure *)
let decide_sq ~(options:options_t) (sqob : sq_obligation) (sqno:int) : bool option =
...
(** print message *)
  ...
  let run (f : form) : bool =
	...
	let coqArgs = ["-I"; Util.lib_name ""; vc_in] in
	let _ = Util.run_with_timeout_out
	    coqName coqArgs vc_out timeout (not !Util.verbose) 
	in
