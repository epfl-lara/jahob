...
let p_invariant (i : invariant_desc) : string =
  ...
  let encap = if i.Specs.inv_encap then "encap " else "" in
  let name = if i.Specs.inv_name <> "" then "\"" ^ i.Specs.inv_name ^ "\" " else "" in
    Printf.sprintf "%s%s%sinvariant %s%s" publicness ensured encap name 
      (PrintForm.quoted_form i.Specs.inv_form)


(** searches for quotes strings and does token replacements there *)
let replace_xsym (a : string) : string = a  

let pretty_print 
    (public_only : bool)
    (xsym : bool)
    (original : string)
    (parsed_specs : Specs.spec list) : string =
  if xsym then replace_xsym original
  else original
(* !!!FIX
    let specs_to_print = List.filter is_public_spec parsed_specs in
    String.concat "\n" (List.map (pp_spec xsym) specs_to_print)
*)
