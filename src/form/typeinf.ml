(**
  Doing some simple type inference for {!Form} formulas. - obsolete
*)
...
and inferHelper (f0 : form) (tenv : typeEnv) : (form * ident list) = match f0 with
  ...
  | App (f, fs) ->
	...
	begin
	  match f with
	    | (Const And | Const Or | Const Not | Const Impl | Const Iff | Const MetaImpl | Const MetaAnd) as appconst -> 
		(* [ bool -> ] bool -> bool *)
	    ...
	    | (Const Cardeq | Const Cardleq | Const Cardgeq | Const Subset | Const Seteq) as appconst ->
		(* set -> set -> bool *)
