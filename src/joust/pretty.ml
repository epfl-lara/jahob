...
let (public_only : bool ref) = ref false
let (use_xsym : bool ref) = ref false

let special_comment s = "/*:" ^ s ^ "*/"
...
let modifier_to_string m =
  ...
  | Native -> "native"
  | AnnotationModifier s -> special_comment s
...
and print_decl f d =
  match d with
    ...
    | AnnotationDecl a -> fprintf f "%s" (special_comment a)
...
and is_public_field fld = 
  List.mem Public fld.f_var.v_mods

and print_field f fld =
  if (not !public_only) || is_public_field fld then
...
and fix_xsymbols a = a (* TODO *)

and print_annotation f a =
  fprintf f "%s" (fix_xsymbols a)
(*
  (PrintSpec.pretty_print 
     !public_only 
     !use_xsym 
     a 
     (ParseSpec.parse_specs a))
*)
...
and print_stmt f stmt =
  ...
  | While (oa, e, st) ->
      (match oa with
	 | None -> fprintf f "while (%a)%a" print_expr e print_body st
	 | Some inv -> 
	     fprintf f "while %a (%a) %a"
	       print_annotation inv print_expr e print_body st)
  | Do (st, e) ->
  ...
  | AnnotationStmt a -> fprintf f "%s" (special_comment a)
