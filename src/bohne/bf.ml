module type BF = 
  sig
    ...
    val top      : t
    val bottom   : t
    val index_of_cube : cube -> index -> int
    ...
    val meet : t list -> t
    val rel_prod : t -> t -> t
    val exists_index : index list -> t -> t 
    val exists : var list -> t -> t 
    val forall : var list -> t -> t 
    val foreach_cube : t -> (cube -> unit) -> unit 
    val fold_cubes : ('a -> cube -> 'a) -> 'a -> t -> 'a 
    val dom : t -> var list
    val unprime_all : t -> t
    val prime_all : t -> t
    val dnf       : t -> index array list
    val count_minterms : t -> float
...
module Bf =
  struct
    ...
    let exists_index vs f =
      let vset = CaddieBdd.fromList bdd_mg vs in
      CaddieBdd.exists vset f

    let exists vs f =
      exists_index (List.rev_map var_index vs) f
    ...
    let fold_cubes fn init f =
      CaddieBdd.bddFoldCubes fn init f
    ...
    let count_minterms f =
      CaddieBdd.bddCountMinterm f !var_count
