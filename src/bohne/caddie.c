...
value caddie_countMinterm(value b, value nvars) {
  CAMLparam2(b, nvars);
  
  if (Bdd_mgr(b) == NULL) {
    raise_mgrError();
  } else {
    CAMLreturn(copy_double(Cudd_CountMinterm(Bdd_mgr(b), Bdd_node(b), Int_val(nvars))));
  }
}
...
value caddie_foldCubes(value fn, value init, value b)
{
  CAMLparam3(fn, init, b);
  CAMLlocal2(carr, acc);
  int i;
  int *cube;
  CUDD_VALUE_TYPE val;
