...
let print_msg (l : int) (msg : out_channel -> unit) =
  if (!debugOn || !debug_level > 0) && l <= !debug_level
  then (msg debug_chan; flush debug_chan)
...
(* Turns off the debug option for the module associated with the module_name. *)
