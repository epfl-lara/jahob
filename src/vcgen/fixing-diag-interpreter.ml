  ...
  try
    (let result = Hashtbl.find sf sa in
       debug_msg ("fetch_from_store_frame: " ^ (string_of_store_addr sa) ^ 
		    " := " ^ (string_of_v result) ^ "\n");
       result)
  with Not_found -> (Util.fail ("fetch_from_store_frame failed to find " ^ (string_of_store_addr sa));
		     raise Not_found)
...
let read_field
  ...
  let err_msg = ("read_field WARNING: " ^ fname ^ " not found in " ^ 
		   (string_of_v v)) in
    match v with
      | OValue r ->
	  (match r.reference with
...
let rec rename_bound_variables (f : form) : form =
  match f with
	...
	let ids0 = List.map Util.fresh_var ids in
...
let assert_form (st : state_t) (f : form) : bool =
  try 
	...
	false
  with Failure s -> Util.amsg("Evaluation of formula caused error.\nError:" ^ s ^ "\nFormula:\n" ^ PrintForm.string_of_form f ^ "\n"); false
...
let evaluate_postconditions
  ...
  let contract = proc.p_header.p_contract in
  let post = 
    if ext then Sast.concretized_post st.prog im proc proc.p_header
	...
	(try
	 ...
	 with NullPointer ->
	   let err = "NullPointerException: " ^ 
	     (PrintAst.pr_basic_command bc) in
	     raise (InterpreterFailure err)
	   | Termination -> ())
  ...
  try
	...
	if (not (bool_of_value (evaluate st f))) then
	  let err = "interpret_bc:\n\tThe following assumption " ^
	    "evaluated to false:\n\t" ^ (PrintAst.pr_basic_command bc) ^
	    "\nContinuing to execute...\n" in
	    print_string err
    | ProcCall pc ->
	  invoke_procedure st pc
  with Failure m -> (Util.amsg("Failed to execute command. Reason:\n" ^ m ^ "\nContinuing anyway.");
		     raise (Failure m))
