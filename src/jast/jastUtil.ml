...
let mk_bool_simpval b = LiteralVal (BoolLiteral b)
...
let mk_aassert (af : Specs.noted_form) : stmt = 
  mkbasic (Assert af)

let mk_anoteThat (af : Specs.noted_form) : stmt = 
  mkbasic (NoteThat af)
(*  mkbasic (NoteThat(Some af.Specs.af_annot,af.Specs.af_form)) *)

let mk_aassume (af : Specs.annot_form) : stmt = 
  mkbasic (Assume(Some af.Specs.af_annot,af.Specs.af_form))

let mk_aassuming (af : Specs.annot_form) : stmt = 
  mkbasic (Assuming(Some af.Specs.af_annot,af.Specs.af_form))

let mk_contradict (af : Specs.annot_form) : stmt =
  mkbasic (Contradict(af.Specs.af_annot, af.Specs.af_form))

let mk_pickAny (pa : Specs.pickAny_cmd) : stmt =
  mkbasic (PickAny pa)

let mk_pickWitness (pa : Specs.pickAny_cmd) : stmt =
  mkbasic (PickWitness pa)

let mk_split (af : Specs.annot_form) : stmt = 
  mkbasic (Split(Some af.Specs.af_annot,af.Specs.af_form))
...
(* let (choose : ident) = Syntax.synth_id "choose" *)
let choice_name = "CHOOSE"
let choice_expr = Syntax.Call(
  Syntax.Name [Syntax.synth_id choice_name], [])
