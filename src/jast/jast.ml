...
type var_decl =
  { vd_name : var_name; (** variable name *)
    ...
    vd_encap : bool * bool; (** if encapsulated, and for arrays, if contents are encapsulated **)
  }
...
let is_this_val : simpval -> bool = function
...
type basic_stmt =
  ...
  | Havoc of Specs.havoc_cmd                   (** non-deterministic asgn *)
  | Assert of Specs.noted_form                 (** assert statement *)
  | NoteThat of Specs.noted_form               (** assert followed by assume *)
  | Assume of string option * Form.form        (** assume statement *)
  | PickAny of Specs.pickAny_cmd               (** pickAny command for proofs *)
  | PickWitness of Specs.pickAny_cmd       (** pickWitness command for proofs *)
  | Induct of Specs.induct_cmd                 (** induct command for proofs *)
  | Contradict of string * Form.form           (** proof by contradiction cmd *)
  | Instantiate of Specs.instantiate_cmd       (** instantiate forall cmd *)
  | Mp of string * Form.form                   (** modus ponens cmd *)
  | Proof                                      (** cmd to create local scope *)
  | Assuming of string option * Form.form      (** assuming command within cases *)
...
type stmt =
  ...
  | Choose of stmt * stmt      (** Non-deterministic choice. *)
...
type method_decl = { 
  ...
  m_encap   : bool; (** does the method modify only private data **)
...
let standard_classes() =
  if !CmdLine.builtinstd then [object_std_class; array_std_class] else []
