...
*)
let block (sts : Syntax.stmt list) : Syntax.stmt =
  Syntax.Block sts
  ...
  | StatProcCall {scall_res = res;
      ...
      let rhs = Syntax.Call(Syntax.Dot(nameof (str2id cn), 
        ...
        (match res with
          | None -> Syntax.Expr rhs
          | Some lv -> asgn (c_lval lv) rhs)
  | Havoc ha -> Syntax.AnnotationStmt (Specs.wr_havoc ha ^ ";")
  | Assert nf -> Syntax.AnnotationStmt (Specs.wr_assert nf ^ ";")
  | NoteThat nf -> Syntax.AnnotationStmt (Specs.wr_noteThat nf ^ ";")
  | Assume(omsg,f) -> Syntax.AnnotationStmt ("assume " ^ 
                                               PrintForm.quoted_form f ^ ";")
  | PickAny pa -> Syntax.AnnotationStmt (Specs.wr_pickAny pa ^ ";")
  | PickWitness pw -> Syntax.AnnotationStmt (Specs.wr_pickAny pw ^ ";")
  | Assuming(omsg,f) -> Syntax.AnnotationStmt ("assuming " ^ 
                                              PrintForm.quoted_form f ^ ";")
  | Induct ic -> Syntax.AnnotationStmt (Specs.wr_induct ic ^ ";")
  | Contradict(msg,f) -> Syntax.AnnotationStmt
      ("contradict " ^ (if msg = "" then "" else msg ^ ": ") ^ 
	 (PrintForm.quoted_form f ^ ";"))
  | Instantiate ic -> Syntax.AnnotationStmt (Specs.wr_instantiate ic ^ ";")
  | Mp(annot,f) -> Syntax.AnnotationStmt 
      ("mp " ^ (if annot = "" then "" else annot ^ ": ") ^ 
	 PrintForm.quoted_form f ^ ";")
  | Proof -> Syntax.AnnotationStmt "proof;"
...
let rec c_stmt (s : stmt) : Syntax.stmt = match s with
  ...
  | Choose(s1,s2) -> Syntax.If(JastUtil.choice_expr, c_stmt s1, 
                             if s2=Block [] then 
                               None 
                             else Some (c_stmt s2))
  | Loop(oinv,s1,c,s2) ->
