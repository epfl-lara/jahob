/* Instantiable hash table built on top of verified association list.
    
    PLUS:
      -instantiable
      -relational interface
      -add, lookup, update operations
      -rehashing on capacity increase
    MINUS:
      -manual setting of ghost field
      -rewrote a loop into tail recursion
      -taken for granted that compute_hash depends 
       only on argument and is deterministic
 */
...
class Hashtable {
    ...
    */
	...
	int hash = compute_hash(key);
        if (!AssocList.member(key, value, table[hash])) size = size + 1;
    ...
    */
    {
	int hash = compute_hash (key);
        size = size - AssocList.countKeys(key, table[hash]);
    ...
    */
	...
	while 
	    /*: inv "content Un l..AssocList.content = (old content) Un (old (bucket..AssocList.content)) 
	      ...
	      & (ALL h. ((h ~= this & h : Hashtable & h : Object.alloc) --> h..size = (old (h..size))))" */ 
	    (! AssocList.is_nil(l)) {
