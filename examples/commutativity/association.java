...
class AssociationList {
    ...
    */
    ...
    {
        Node current = first;
        while /*: inv "(\<exists> v. (k0, v) \<in> content)
                        = (\<exists> v. (k0, v) \<in> current..cnt)" */
            (current != null) {
            if (current.key == k0) {
                return true;
            }
    ...
    /*: requires "k0 \<noteq> null \<and> v0 \<noteq> null
                  \<and> \<not>(\<exists> v. (k0, v) \<in> content)"
        modifies content
    ...
    /*: requires "theinvs \<and> k0 \<noteq> null \<and> v0 \<noteq> null
                  \<and> \<not>(\<exists> v. (k0, v) \<in> content)"
        modifies content, "new..key", "new..value", "new..next", "new..cnt",
                 first
        ensures "theinvs \<and> content = old content \<union> {(k0, v0)}
                 \<and> (\<forall> alist. alist \<in> AssociationList
                                   \<and> alist \<in> Object.alloc
                            \<longrightarrow> alist \<in> old Object.alloc)"
     */
    {
	Node n = new Node();
	n.key = k0;
    ...
    /*: requires "k0 \<noteq> null \<and> v0 \<noteq> null
                  \<and> (\<exists> v. (k0, v) \<in> content)"
        modifies content
	ensures "content = old content - {(k0, result)} \<union> {(k0, v0)}
                 \<and> (k0, result) \<in> old content"
     */
    {
    ...
    /*: requires "theinvs \<and> k0 \<noteq> null
                  \<and> (\<exists> v. (k0, v) \<in> content)"
        modifies content, "Node.next", "Node.cnt"
        ensures "content = old content \<setminus> {(k0, result)}
                 \<and> (k0, result) \<in> old content \<and> theinvs
                 \<and> Object.alloc = old Object.alloc"
    */
    {
	...
	if (f.key == k0) {
	    ...
	    while /*: inv "
                    prev \<noteq> null \<and> current \<noteq> null \<and>
		    prev..cnt = prev..(old cnt) - {(k0, v0)} \<and>
    ...
    public void add2(Object k0, Object v0, Object k1, Object v1)
    /*: requires "k0 \<noteq> null \<and> v0 \<noteq> null
                  \<and> \<not>(\<exists> v. (k0, v) \<in> content)
                  \<and> k1 \<noteq> null \<and> v1 \<noteq> null
                  \<and> \<not>(\<exists> v. (k1, v) \<in> content)
                  \<and> k0 \<noteq> k1"
        modifies content
        ensures "content = old content \<union> {(k0, v0), (k1, v1)}" */
    {
	_add(k0, v0);
	_add(k1, v1);
    }

    public void remove2 (Object k0, Object k1)
    /*: requires "k0 \<noteq> null \<and> (\<exists> v0. (k0, v0) \<in> content)
          \<and> k1 \<noteq> null \<and> (\<exists> v1. (k1, v1) \<in> content)
          \<and> k0 \<noteq> k1"
        modifies content, "Node.next", "Node.cnt"
        ensures "\<exists> v0 v1. (k0, v0) \<in> old content
                                  \<and> (k1, v1) \<in> old content
                  \<and> content = old content \<setminus> {(k0, v0), (k1, v1)}"
    */
    {
        _remove(k0);
        _remove(k1);
