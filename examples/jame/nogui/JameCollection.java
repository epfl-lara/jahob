...
public class JameCollection
{
    /*: 
      ...
      vardefs "collection == 
         {c. EX n. c = n..JameNode.content & rtrancl_pt (% x y. JameNode.next x = y) head n}";
     ...
     // starting from current 
      ...
      vardefs "free == 
         {c. EX n. c = n..JameNode.content & rtrancl_pt (% x y. JameNode.next x =y) current n}";

     invariant isTree: "tree[JameNode.next]";

     public invariant freeSubCollection: "free <= collection";
     
     invariant prev: "ALL x y. JameNode.prev x = y --> (y ~= null --> JameNode.next y = x) & 
                               (y = null & x ~= null --> (ALL z. JameNode.next z ~= x))";
    ...
    */
    ...
    /*:       
     ensures "result..collection = this..collection &
              result : (Object.alloc \<setminus> old Object.alloc)"
     */
    {
        JameCollection clone = new JameCollection();
        this.resetIter();        
        while /*: inv "clone..collection = collection \<setminus> free & 
	               free <= collection & 
		       clone : (Object.alloc \<setminus> old Object.alloc)"
	      */
        ( this.hasNext() )
        {        	
            clone.add( next() );
