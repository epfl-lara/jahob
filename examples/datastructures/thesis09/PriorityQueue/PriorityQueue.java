...
public class PriorityQueue
{
    ...
    /*:
      ...
      invariant CapacityInv: "init \<longrightarrow> 0 < capacity";
      invariant InitialLength: "\<not>init \<longrightarrow> length = 0";
      invariant CardInv: "init \<longrightarrow> length = card(contents)"
      invariant LengthInv: "init \<longrightarrow> 0 \<le> length \<and> length \<le> queue..Array.length";
      ...
      invariant OrderedInv: "init \<longrightarrow> (\<forall> i j.
       (0 \<le> i \<and> i < length \<and> 0 \<le> j \<and> j < length \<and> (j=2*i+1 \<or> j=2*i+2)
      ...
      "\<forall> e1 e2. e1 : elements & e2 : elements & e1 ~= e2 --> (e1..ob ~= e2..ob | e1..pr ~= e2..pr)";
    ...
    */
    ...
    /*: requires "0 < i"
        ensures "0 \<le> result \<and> result < i \<and>
	         (i = 2 * result + 1 \<or> i = 2 * result + 2) \<and> 
		 alloc = old alloc" */
    {
	return (i-1)/2;
    }
    ...
    /*: requires "0 \<le> i"
        ensures "i < result \<and> result = 2 * i + 1 \<and> alloc = old alloc" */
    ...
    /*: requires "0 \<le> i"
        ensures "i < result \<and> result = 2 * i + 2 \<and> alloc = old alloc" */
    {
	return (2*i + 2);
    }
	...
	ensures "length < capacity \<and> 
	         (\<forall>x. x \<in> PriorityQueue \<longrightarrow> x..contents = old (x..contents)) \<and> 
	         (\<forall>x. x \<in> PriorityQueue \<longrightarrow> x..init = old (x..init)) \<and> 
	         (\<forall>a i. a \<noteq> queue \<longrightarrow> a.[i] = old (a.[i])) \<and> theinvs" */
    {
    ...
    */
    ...
    /*: requires "init \<and> o1 \<noteq> null \<and> (o1, p1) \<notin> contents \<and> 
                  length < capacity \<and> theinvs"
	modifies contents, length, arrayState, ob, pr
	ensures "contents = old contents \<union> {(o1, p1)} \<and> 
	         length = old length + 1 \<and> 
		 (\<forall>x. x \<in> old alloc \<and> x \<in> PriorityQueue \<and> x \<notin> hidden \<and> 
		 x \<noteq> this \<longrightarrow> x..contents = old (x..contents)) \<and>
		 (\<forall>a i. a \<noteq> queue \<longrightarrow> a.[i] = old (a.[i])) \<and> theinvs" */
    {
	int i = length;
	length = length + 1;
			 ...
			 ((i + i + 2 < length) -->
			  (queue.[(i + i + 2)]..PQElement.pr <
		       ...
		       (old elements = { p. EX j. 0 <= j & j < length &
                                          j ~= i & p = queue.[j] })) &
		       (\<forall> j. 0 <= j & j < length & j ~= i --> 
		               queue.[j] ~= null) & 
		       (comment ''PDBefore''
		        (\<forall> j k. 0 <= j & j < length & 0 <= k & k < length &
	    ...
	    (i > 0 && queue[parent(i)].pr < e.pr)
	{
	    ...
	    /*: note iEffect1: "\<forall> jj. (0 \<le> jj \<and> jj < length \<and> 
	        ((i = jj + jj + 1) | (i = jj + jj + 2)) \<longrightarrow> 
	        queue.[jj]..pr \<ge> queue.[i]..pr)"; */

	    /*: note iEffect2: "\<forall> kk. (0 \<le> kk \<and> kk < length \<and>
	        ((kk = i + i + 1) | (kk = i + i + 2)) \<longrightarrow>
	        queue.[i]..pr \<ge> queue.[kk]..pr)" 
	        from OrderedPostLoop, InLoop, PIRel, PBounds; */

	    /*: note OtherEffect: 
	      "\<forall> jj kk. (0 \<le> jj \<and> jj < length \<and> 0 \<le> kk \<and> kk < length \<and> 
	    ...
	    /*: noteThat sameAfter:
	      "\<forall> jj kk. (0 <= jj & jj < length & 0 <= kk & kk < length & 
	...
	{
	    //: localize;
	    //: note ContentsForw: "\<forall> x y. (x, y) : contents --> (x, y) : old contents Un {(o1, p1)}" from contents_def, Hyp, NewElements;
	    {
		//: pickAny x::obj, y::int;
		{
...

	{
		...
		}
		{
		    ...
		    //: note NotThisConc: "\<forall> e1 e2. e1 : pq..elements & e2 : pq..elements & e1 ~= e2 --> (e1..ob ~= e2..ob | e1..pr ~= e2..pr)";
		}
		//: note ContentsInjConc: "\<forall> e1 e2. e1 : pq..elements & e2 : pq..elements & e1 ~= e2 --> (e1..ob ~= e2..ob | e1..pr ~= e2..pr)" from ContentsThis, NotThisConc;
	    }
	    //: note ContentsInjPost: "pq : alloc & pq : PriorityQueue --> (\<forall> e1 e2. e1 : pq..elements & e2 : pq..elements & e1 ~= e2 --> (e1..ob ~= e2..ob | e1..pr ~= e2..pr))" forSuch pq;
	}
	{
	    //: pickAny x::obj;
	    {
    ...
    /*: requires "init \<and> 0 < length \<and> theinvs"
        ensures "(\<forall>k. 0 \<le> k \<and> k < length \<longrightarrow> queue.[0]..pr \<ge> queue.[k]..pr) \<and> theinvs" */
    {
	{
		...
		{
		    //: pickAny x::int suchThat "x \<le> (kk + 1) \<and> 0 \<le> x \<and> x < length";
		    ...
		    //: note InductConc: "queue.[0]..pr \<ge> queue.[x]..pr" forSuch x;
		}
	...
	/*: note Found: "(queue.[0]..ob, queue.[0]..pr) \<in> contents"
	  from ProcedurePrecondition, FalseBranch, thisType, elements_def, contents_def; */
    ...
    /*: requires "init"
        ...
        ensures  "(old contents = {} --> 
	            contents = old contents \<and> result = null) \<and>
	          (old contents ~= {} -->
	...
	//: note Found: "(elem..ob, elem..pr) \<in> old contents" from ProcedurePrecondition, LengthInv, FalseBranch, thisType, elements_def, contents_def;

	{
	    //: localize;
	    //: note Init: "init";
	    //: note ElemInOld: "elem \<in> old elements" from Init, LengthInv, FalseBranch, thisType, elements_def;
	...
	//: note IsMax: "\<forall> x y. (x, y) : old contents --> elem..pr >= y";
	/*: witness "elem..pr" for
	  PostCond: "EX pr. (elem..ob, pr) \<in> old contents \<and> contents = old contents - {(elem..ob, pr)} & (\<forall> x y. (x, y) : (old contents) --> pr >= y)"
...

	{
		...
		//: note ConcForw: "\<forall> x y. (x, y) : pq..contents --> (x, y) : (fieldRead (old PriorityQueue.contents) pq)";
		//: note ConcBack: "\<forall> x y. (x, y) : (fieldRead (old PriorityQueue.contents) pq) --> (x, y) : pq..contents";
    ...
    /*: requires "init \<and> 0 \<le> i \<and> i < length \<and> 
		  ...
		  (\<forall>k j.(0 \<le> k \<and> k < length \<and> k \<noteq> i \<and> 0 < j \<and> j < length \<and> 
	            ((j = 2 * k + 1) \<or> (j = 2 * k + 2)) \<longrightarrow> 
		    queue.[k]..pr \<ge> queue.[j]..pr)) \<and> 
		  comment ''LocalOrderingPre''
		  (\<forall>x.((0 \<le> x \<and> (i = 2 * x + 1 \<or> i = 2 * x + 2)) \<longrightarrow> 
		    (((2 * i + 1 < length) \<longrightarrow> 
		     queue.[x]..pr \<ge> queue.[(2 * i + 1)]..pr) \<and> 
		     ((2 * i + 2 < length) \<longrightarrow> 
		     queue.[x]..pr \<ge> queue.[(2 * i + 2)]..pr)))) \<and> 
		  comment ''OrderedFrame'' 
		  (\<forall>pq. pq \<in> PriorityQueue \<and> pq \<in> alloc \<and> pq..init \<and> 
		    pq \<noteq> this \<longrightarrow> 
		    (\<forall>i j. 0 \<le> i \<and> i < pq..length \<and> 0 \<le> j \<and> 
		      j < pq..length \<and> (j = 2 * i + 1 \<or> j = 2 * i + 2) \<longrightarrow> 
		      pq..queue.[i]..pr \<ge> pq..queue.[j]..pr)) \<and>
		  theinv CardInv \<and> theinv CapacityInv \<and> 
		  theinv InitialLength \<and> theinv LengthInv \<and> 
		  theinv NonNullInv \<and> theinv DistinctInv \<and> 
		  theinv NullInv \<and> theinv HiddenInv \<and> theinv InjInv \<and> 
		  theinv ContentsInj"
        modifies arrayState
